...
static void usage(const char *argv0) {
    fprintf(stderr,
        "kewld v%s — The Kewl Onion Daemon\n"
        "\n"
        "Usage: %s [options]\n"
        "\n"
        "Options:\n"
        ...
        "  --data-dir     <path>    Data directory (default: ~/.kewld/<tag>)\n"
        "  --port         <port>    Local HTTP port (default: %d)\n"
        "  --admin-port   <port>    Local admin UI port (default: %d)\n"
        "  --admin-token  <token>   Bearer token for admin UI (recommended)\n"
        "  --socks-port   <port>    Tor SOCKS port (default: %d)\n"
        "  --ctrl-port    <port>    Tor control port (default: %d)\n"
        "  --no-media               Disable media uploads\n"
        "  --nsfw                   Mark board as NSFW\n"
        "  --no-register            Don't register with kewl index\n"
        ...
        "\n"
        "Example:\n"
        "  kewld --tag tech --title 'Technology' --desc 'Tech discussion' \\\n"
        "        --admin-token mysecrettoken\n"
        "\n"
        "Admin UI: http://127.0.0.1:<admin-port>/admin/\n"
        "  (local only — never accessible via Tor or network)\n",
        KEWLD_VERSION, argv0,
        KEWLD_DEFAULT_HTTP_PORT,
        KEWLD_DEFAULT_ADMIN_PORT,
...
static void config_defaults(kewld_config_t *cfg) {
    ...
    cfg->admin_port     = KEWLD_DEFAULT_ADMIN_PORT;
    ...
    strncpy(cfg->desc,  "A kewl board.",   sizeof(cfg->desc)-1);
}
...
#define NEED_ARG(flag) \
    if (i+1>=argc) { fprintf(stderr,"error: %s requires an argument\n",flag); return -1; } i++
        ...
        if (strcmp(argv[i],"--help")==0||strcmp(argv[i],"-h")==0) {
            ...
            char *p=cfg->tag; while(*p=='/') memmove(p,p+1,strlen(p));
            ...
            NEED_ARG("--admin-port");
            cfg->admin_port=(uint16_t)atoi(argv[i]);
        } else if (strcmp(argv[i],"--admin-token")==0) {
            NEED_ARG("--admin-token");
            strncpy(cfg->admin_token, argv[i], sizeof(cfg->admin_token)-1);
        ...
        } else if (strcmp(argv[i],"--no-media")==0) {
            cfg->allow_media=0;
        } else if (strcmp(argv[i],"--no-images")==0) {  /* legacy alias */
            cfg->allow_media=0;
...
/* Thread that runs the admin server so both servers run concurrently */
static void *admin_thread_fn(void *arg) {
    kewld_admin_server_t *asrv = (kewld_admin_server_t *)arg;
    admin_server_run(asrv);
    return NULL;
}
...
int main(int argc, char **argv) {
    ...
    if (cfg.data_dir[0] == '\0') build_default_data_dir(&cfg);

    if (!cfg.admin_token[0])
        log_warn("WARNING: no --admin-token set; admin UI is unprotected!");

    log_info("kewld v%s starting", KEWLD_VERSION);
    ...
    if (cfg.daemon) daemonize(cfg.pid_file[0] ? cfg.pid_file : NULL);
    ...
    if (tor_launch(&cfg) != 0) { log_err("failed to launch Tor"); return 1; }
    if (tor_wait_ready(cfg.tor_ctrl_port, 120) != 0) {
        log_err("Tor did not become ready in time"); return 1;
    }
    if (tor_read_onion(tor_dir, cfg.onion_addr, sizeof(cfg.onion_addr)) != 0) {
        log_err("could not read onion address"); return 1;
    }

    log_info("onion address : %s", cfg.onion_addr);
    log_info("board URL     : http://%s/%s/", cfg.onion_addr, cfg.tag);

    /* Register with kewl index */
    if (cfg.register_index) {
        log_info("registering with kewl index at %s ...", KEWLD_INDEX_HOST);
        if (index_register(&cfg) != 0)
            log_warn("registration failed (continuing anyway)");
        else
            log_info("submitted for review to index.");
    }

    /* Start admin server in background thread (127.0.0.1 only, never Tor) */
    kewld_admin_server_t *asrv = admin_server_create(&cfg, db);
    if (!asrv) {
        log_err("failed to create admin server");
        return 1;
    }
    pthread_t atid;
    if (pthread_create(&atid, NULL, admin_thread_fn, asrv) != 0) {
        log_err("failed to start admin thread");
        return 1;
    }
    pthread_detach(atid);
    log_info("admin UI: http://127.0.0.1:%d/admin/ (local only)", cfg.admin_port);

    /* Start public HTTP server (blocks until shutdown) */
    kewld_server_t *srv = http_server_create(&cfg, db);
    if (!srv) { log_err("failed to create HTTP server"); return 1; }
    log_info("HTTP server listening on 127.0.0.1:%d", cfg.http_port);
    http_server_run(srv);

    http_server_destroy(srv);
    admin_server_destroy(asrv);
    sqlite3_close(db);
    tor_shutdown();
    log_info("kewld stopped.");
    return 0;
}
